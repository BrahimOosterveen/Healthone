...
<div class="container">
    ...
    <main>
        <article>
            ...
            $sql = "SELECT id, Naam, Achternaam, Geboortedatum, status FROM Apotheker WHERE status = 1";
            $result = mysqli_query($conn, $sql);

            if($result = mysqli_query($conn, $sql)){
                if(mysqli_num_rows($result) > 0){
                    echo "<input type=\"text\" id=\"myInput\" onkeyup=\"myFunction()\" placeholder=\"Search for ID..\">";
                    echo "<h4 class='shadow' style='padding: 10px;'>Opgehaalde recepten | Patienten</h4>";
